...
slack = Slacker('xoxb-1593742025719-1601739413350-rblkSNktL6RoGfh3VJspdFqg')
def dbgout(message):
    """인자로 받은 문자열을 파이썬 셸과 슬랙으로 동시에 출력한다."""
    print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message)
    strbuf = datetime.now().strftime('[%m/%d %H:%M:%S] ') + message
    # 내가 만든 채널 이름 붙이기 #stock
...
def get_target_price(code):
    ...
    try:
        ...
        # 지난 날 고가/저가 계산하고 k(변동성)만큼 올랐으면 목표가 , k값을 바꾸는 것도 전략
...
def buy_etf(code):
    ...
    try:
        ...
        if code in bought_list: # 매수 완료 종목이면 더 이상 안 사도록 함수 종료
            ...
            return False
        time_now = datetime.now()
        current_price, ask_price, bid_price = get_current_price(code) # 현재 가격 구하기
        target_price = get_target_price(code)    # 매수 목표가 _ 전략#1
        ma5_price = get_movingaverage(code, 5)   # 5일 이동평균가 _ 전략#2
        ma10_price = get_movingaverage(code, 10) # 10일 이동평균가 _ 전략#3
        ...
        if current_price > target_price and current_price > ma5_price \
            and current_price > ma10_price:  # 전략#1/2/3 모두 만족하는지 확인!
            ...
            if bought_qty > 0: # 구매했으면 
                bought_list.append(code) # bought_list에 추가 
                dbgout("`buy_etf("+ str(stock_name) + ' : ' + str(code) + 
                    ") -> " + str(bought_qty) + "EA bought!" + "`") # slack 메시지 출력
    except Exception as ex:
...
if __name__ == '__main__': 
    try:
        ...
        buy_percent = 0.19 # 종목당 몇 %씩 살 건지 ex) 4종목이면 25% 작성하면 균등하게 100% 구매 
        ...
        while True:
            ...
            t_start = t_now.replace(hour=9, minute=5, second=0, microsecond=0) # 자동매매 시간
            t_sell = t_now.replace(hour=15, minute=15, second=0, microsecond=0) # 팔기시작 시간
            t_exit = t_now.replace(hour=15, minute=20, second=0,microsecond=0) # 프로그램 종료
            today = datetime.today().weekday()
            if today == 5 or today == 6:  # 토요일이나 일요일이면 자동 종료 _ 시장이 안열리기 때문에 종료
                printlog('Today is', 'Saturday.' if today == 5 else 'Sunday.')
                sys.exit(0)
            if t_9 < t_now < t_start and soldout == False: # 시간이 9시 ~ 9시 5분 사이 남아있는 주식 있다면 모든 주식 팔아라
                soldout = True
                sell_all()
            if t_start < t_now < t_sell :  # AM 09:05 ~ PM 03:15 : 매수
                for sym in symbol_list:
                    if len(bought_list) < target_buy_count: # 목표 종목수 보다 덜 샀으면
                        buy_etf(sym) # etf 구매 
                        time.sleep(1)
                if t_now.minute == 30 and 0 <= t_now.second <= 5: # 30분마다 잔고 및 금액 표출 
